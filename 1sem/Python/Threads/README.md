Ex01.py Fazer uma aplicação que rode 5 Threads que cda uma delas receba um inteiro chamado id como
parâmetro e imprima no console o texto "Thread #" + id. Antes de imprimiro valor, 
deve-ser fazer um sleep de 0.5 segundos.

Ex02.py Fazer uma aplicação que, na main, inicialize uma variável id, inteirae inicialize 5 variáveis
inteiras para valores, crie um vetor de paraâmetros, com o id como primeiro parâmetro e um vetro com os 5 valores recebidos. As variáveis que recebem os valores devem receber, cada uma delas, um valor aleatório
de 1 a 100. Esses parâmetros devem ser preenchidos para 3 chamdas de Threads. Faça 3 chamadas de Threads,
passando os parâmetros e, cada Thread, deve calcular a soma de cada linha (Cada iteração do laço, par a soma deve ser seguido por um sleep de 0.2 segundos), ao final, deve-se imprimir a identificação da linha
e o resultado da soma.

Ex03.py Fazer uma aplicação de uma corrida de sapos, com 5 Threads, cada Thread controlando 1
...
Ex04.py No Sistema Operacional Linux, o comando para realizar uma operação de ping com 10
